...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                        ...
                                        ] + [{'label': site, 'value': site} for site in spacex_df['Launch Site'].unique()],  # Add options for each launch site
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        # Filter the data for successful launches only
...
def update_scatter_chart(selected_site, payload_range):
    ...
    # Filter the dataset by payload range
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= low) &
                            (spacex_df['Payload Mass (kg)'] <= high)]
    
    if selected_site == 'ALL':
        # If all sites are selected, include all data
        fig = px.scatter(
